Shader "Hidden/ChocDino/UIFX/ErodeDilate"
{
	...
	float4 Dilate(v2f i, float2 texelSize)
	{	
		float4 result = 0.0;
		...
		for (int j = -m; j <= m; j++)
		{
			...
			result = max(result, lerp(sample, result, remainder));
		}

		return result;
	}

	float DilateAlpha(v2f i, float2 texelSize)
	{	
		float result = 0.0;
	...
	float ErodeAlpha(v2f i, float2 texelSize)
	{	
		float result = 1.0;
	...
	float ErodeDilateAlpha(v2f i, float2 texelSize)
	{	
	...
	float4 fragDilate(v2f i) : SV_Target
	{
		...
		return Dilate(i, _MainTex_TexelSize);
	}

	float4 fragDilateAlpha(v2f i) : SV_Target
	{
		return DilateAlpha(i, _MainTex_TexelSize);
	}

	float4 fragErodeAlpha(v2f i) : SV_Target
	{
		return ErodeAlpha(i, _MainTex_TexelSize);
	}

	float4 fragErodeDilateAlpha(v2f i) : SV_Target
	{
		return ErodeDilateAlpha(i, _MainTex_TexelSize);
	}
