...
namespace Borodar.RainbowFolders.Editor
{
    public class RainbowFoldersPopup : DraggablePopupWindow
    {
        ...
        public void ShowWithParams(Vector2 position, List<string> paths, int pathIndex)
        {
            ...
            _currentFolder = new RainbowFolder(RainbowFolder.KeyType.Path, paths[pathIndex]);
        ...
        public override void OnGUI()
        {
            ...
            _currentFolder.Type = (RainbowFolder.KeyType)EditorGUI.EnumPopup(rect, _currentFolder.Type);
            ...
            GUI.enabled = false;
            if (_paths.Count == 1)
                _currentFolder.Key = (_currentFolder.Type == RainbowFolder.KeyType.Path) ? _paths[0] : Path.GetFileName(_paths[0]);
            else
                _currentFolder.Key = "---";
        ...
        private void ButtonApply(Rect rect)
        {
            ...
            {
                _currentFolder.Key = (_currentFolder.Type == RainbowFolder.KeyType.Path)
                    ? _paths[i]
